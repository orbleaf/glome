...
void * m_first_chunk(void) {
	return (void *)((intptr)alloc_buffer + (intptr)sizeof(alloc_chain));
}

void * m_next_chunk(void * chunk) {
	alloc_chain * alloc_ptr = (alloc_chain *)((intptr)chunk - (intptr)sizeof(alloc_chain));
	alloc_ptr = alloc_ptr->next;
	if(alloc_ptr->size == 0) return NULL;
	return (void *)((intptr)alloc_ptr + (intptr)sizeof(alloc_chain));
}

uint32 m_size_chunk(void * chunk) {
	alloc_chain * alloc_ptr = (alloc_chain *)((intptr)chunk - (intptr)sizeof(alloc_chain));
	return alloc_ptr->size;
}

void * m_shift_next_chunk(void * chunk) {
	alloc_chain * alloc_ptr = NULL;
	alloc_chain * next_ptr = NULL;
	intptr gap = 0;
	if(chunk == NULL) return NULL;
	alloc_ptr = (alloc_chain *)((intptr)chunk - (intptr)sizeof(alloc_chain));
	next_ptr = alloc_ptr->next;
	gap = (intptr)next_ptr - ((intptr)alloc_ptr + sizeof(alloc_chain) + alloc_ptr->size);
	if(gap > 0) {
		alloc_ptr->next = (alloc_chain *)((intptr)alloc_ptr + sizeof(alloc_chain) + alloc_ptr->size);
		memcpy(alloc_ptr->next, next_ptr, next_ptr->size + sizeof(alloc_chain));
		next_ptr = alloc_ptr->next;
	}
	if(next_ptr->size == NULL) return NULL;
	return (void *)((intptr)next_ptr + (intptr)sizeof(alloc_chain));
}
/*void main()
{
	Heap_Manager * h1;
