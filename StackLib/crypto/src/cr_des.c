 ...
 * modified to support decryption Agus Purwanto
 *
 ...
 */
uint8_t KeyRotation[16] =
  { 1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1 };

uint8_t KeyRotationRight[] = 
...
extern void print_hex(BYTE * buffer, WORD length);
void KeyShiftRight( uchar *key, int numbits )
  /* ------------------------------------------------------------------------ **
  ...
  {
  int   i,j;
  uchar keep = key[6];  /* Copy the highest order bits of the key. */
  ...
  for( i = 0; i < numbits; i++ )
    ...
    for( j = 7; j > 0; j-- )
      {
      if( j != 7 && (key[j-1] & 0x01) )  /* If the top bit of this byte is set. */
        key[j] |=  0x80;        /* ...shift it to last byte's low bit. */
      key[j-1] >>= 1;               /* Then left-shift the whole byte.     */
      }
     ...
     */
    if( GETBIT( key, 28 ) )     /* If bit 27 is set... */
      {
      CLRBIT( key, 28 );        /* ...clear bit 27. */
      SETBIT( key, 0 );        /* ...set lowest order bit of 2nd half-key. */
      }
...
void KeyShift( uchar *key, int numbits )
  /* ------------------------------------------------------------------------ **
   * Split the 56-bit key in half & left rotate each half by <numbits> bits.
   *
   *  Input:  key     - The 56-bit key to be split-rotated.
...
static void sbox( uchar *dst, uchar *src )
  /* ------------------------------------------------------------------------ **
...
static void xor( uchar *dst, uchar *a, uchar *b, int count )
  /* ------------------------------------------------------------------------ **
...
uchar * cr_convert_key( uchar *dst, uchar *key ) {
  /* ------------------------------------------------------------------------ **
...
void cr_des_encode( BYTE *dst, BYTE *dkey, BYTE *src ) _REENTRANT_ {
  /* ------------------------------------------------------------------------ **
...
//ALGORITMA DEKRIPSI GAGAL!!!
void cr_des_decode( BYTE *dst, BYTE *dkey, BYTE *src ) _REENTRANT_ {
  /* ------------------------------------------------------------------------ **
   * DES encryption of the input data using the input key.
   *
   *  Input:  dst - Destination buffer.  It *must* be at least eight bytes
  ...
  for( i = 0; i < 16; i++)
  {
    ...
    /* Generate the subkey for this round.
     */
    KeyShiftRight( K, KeyRotationRight[i] );
    Permute( SubK, K, KeyCompression, 6 );
...
{
	cr_des_encode( dst, key, src );
	cr_des_decode( dst, key + 8, dst );
	cr_des_encode( dst, key, dst );
}

//implementasi algoritma dekripsi triple DES
void cr_tdes2_decode(BYTE *dst, BYTE *key, BYTE *src )
{
	cr_des_decode( dst, key, src );
	cr_des_encode( dst, key + 8, dst );
	cr_des_decode( dst, key, dst );
}
...
void cr_tdes3_encode(BYTE *dst, BYTE *key, BYTE *src )
{
	cr_des_encode( dst, key, src );
	cr_des_decode( dst, key + 8, dst );
	cr_des_encode( dst, key + 16, dst );
}

//implementasi algoritma dekripsi triple DES
void cr_tdes3_decode(BYTE *dst, BYTE *key, BYTE *src )
{
	cr_des_decode( dst, key + 16, src );
	cr_des_encode( dst, key + 8, dst );
	cr_des_decode( dst, key, dst );
}
...
{	
	...
	//karena sebab yang belum jelas buffer harus diisi dengan memcopy
	//sebab jika diisi dengan buffer[]="12345678" maka proses dekripsi DES tidak berjalan
	memcopy(buffer, "12345678", 0, 8 );
	...
	printf("\n");
	auth_DEShash(buffer_2, CHV_KEY, buffer );
	printf("DES encrypted text : ");
	for(i=0;i<8;i++){
		printf("%c", buffer_2[i]);
	}
	printf("\n");
	decrypt_DEShash(buffer_3, CHV_KEY, buffer_2 );
	printf("DES decrypted text : ");
	for(i=0;i<8;i++){
		printf("%c", buffer_3[i]);
	}
	printf("\n");
	...
	printf("\n");
	auth_3DES(buffer_2, CHV_KEY, buffer );
	printf("3DES encrypted text : ");
	for(i=0;i<8;i++){
		printf("%c", buffer_2[i]);
	}
	printf("\n");
	decrypt_3DES(buffer_3, CHV_KEY, buffer_2 );
